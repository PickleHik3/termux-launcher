...
# pasted it rather than assuming one, then adds the oh-my-posh init for
# that shell only: bash/zsh via a marker block in their rc file, fish via
# its own conf.d drop-in. Idempotent: rerunning changes nothing.
set -euo pipefail
...
add_init_block() {
    ...
    if [ -f "$rc_file" ] && grep -q 'oh-my-posh init' "$rc_file"; then
        echo "launcher-material: $rc_file already runs its own oh-my-posh init; installing ours too, the last init wins"
    fi
    ...
    echo "launcher-material: added the oh-my-posh init to $rc_file"
}

case "$shell_name" in
    bash)
        add_init_block "$HOME/.bashrc" "eval \"\$(oh-my-posh init bash --config \"$rendered\")\""
        echo "Open a new terminal (or run 'exec bash') to see the prompt."
        ;;
    zsh)
        add_init_block "$HOME/.zshrc" "eval \"\$(oh-my-posh init zsh --config \"$rendered\")\""
        echo "Open a new terminal (or run 'exec zsh') to see the prompt."
        ;;
    fish)
        config_fish="${XDG_CONFIG_HOME:-$HOME/.config}/fish/config.fish"
        if [ -f "$config_fish" ] && grep -qE 'oh-my-posh.*init[[:space:]]+fish' "$config_fish"; then
            echo "launcher-material: $config_fish already initialises oh-my-posh; nothing more is needed."
            exit 0
        fi
        ...
        if [ -f "$fish_init" ] && cmp -s "$fish_init" "$tmp_file"; then
            ...
            rm -f "$tmp_file"
            echo "launcher-material: added the oh-my-posh init to $fish_init"
        fi
