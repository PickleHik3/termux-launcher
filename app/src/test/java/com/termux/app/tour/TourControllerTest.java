...
public class TourControllerTest {
    ...
    private void aRunWithAStageThatIsOnlyShown() {
        ...
        run.set(run.indexOf(step(TourRun.KEYBOARD, run)), shown);
        controller = new TourController(run, prefs, clock);
        controller.setListener(listener);
    }

    private static TourStep step(String id, List<TourStep> run) {
        for (TourStep step : run) if (step.id.equals(id)) return step;
