...
| C4 | **Glyph strength 30 %** of the text colour, +10 points when selected. |
...
- **Glyph** = today's `processGlyph` map rendered from the Nerd Font face into a per-chip layered
  drawable under the text: 16 dp, text colour at 30 % (accent-tinted at 40 % when selected),
  centred, clipped to the chip. The strength is a constant, not a setting.
