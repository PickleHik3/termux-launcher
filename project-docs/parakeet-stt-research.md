...
Parakeet TDT 0.6B v3 is a second on-device engine next to Whisper, on the LiteRT route above.

- **Catalog:** id `parakeet-tdt-0.6b-v3` (`TaiModelCatalog.PARAKEET_TDT_V3_ID`), architecture
  `parakeet-tdt`, capability `speech_to_text`, RAM tier 8GB+, licence **CC-BY-4.0** (NVIDIA's
  weights; the litert-community repo carries an apache-2.0 tag but converts those weights).
  Graph `litert-community/parakeet-tdt-0.6b-v3` @ `50dae0cb8c7b39dda477966eff7150cd7fe206ae`,
  `parakeet_tdt_0.6b_v3_5s_i8_stateful.tflite`, 614,261,072 B, sha256
  `334745b8bc7fd372b1c213516f0b6338bb827b1a2abb3e77ad35fe6fea5cd16b` (HF LFS metadata). Tokenizer
  sidecar `nvidia/parakeet-tdt-0.6b-v3` @ `541d1f99c6b0c3cd0b11a95167540bb8edefd82b`
  `tokenizer.json`, 1,159,960 B, sha256 `bd321b096832a3f270bd3b2a88823957920f1a5c5ada71114a26ea729d0cbe91`
  (hashed after download; it is a plain git blob, not LFS). One 5 s window, no window choice.
- **Classes** (`com.termux.ai`): `SttRuntime` (the engine interface `MultiBackendTaiRuntime`
  routes to, by architecture / file name; the other engine is closed first so one STT graph is
  resident), `ParakeetSttRuntime` (interpreter, signatures bound by shape, XNNPACK shim, residency),
  `ParakeetFeatures` (NeMo log-mel, audio padded to 5 s with seeded −80 dBFS noise),
  `ParakeetTdtDecoder` (the greedy TDT loop over a small `Graph` interface), `ParakeetTokenizer`
  (decode only). Pieces are cut with `WhisperSegmenter.splitPoints` at the 5 s window, no drop or
  pad. Residency estimate: 2.0× the file (`TaiResidency.PARAKEET_STT_FACTOR_TENTHS`), from the
  +1.2 GB RSS measured above. `language` and `prompt` are accepted and ignored (`biased: false`).
- **Tests:** `ParakeetFeaturesTest` against `app/src/test/resources/parakeet/` (generated by
  `gen_fixture.py` there from `scripts/parakeet_replay_server.py`), `ParakeetTdtDecoderTest`,
  `ParakeetTokenizerTest`, `ParakeetSttRuntimeTest`.
- **Selecting it:** Keyboard → Voice input → Speech model → Download a model → Engine: Parakeet
  (Whisper keeps its size/language/window questions). A phone under 8 GB gets a warning under the
  choice. Installed rows show the engine in their summary; the model activates when the download
  lands, like Whisper.
- **Attribution:** the app has no third-party model licence screen (the chat catalog shows each
  entry's licence in its download dialog; speech entries are not listed there), so the CC-BY-4.0
  credit lives in the catalog entry's `license` field and here: NVIDIA Parakeet TDT 0.6B v3,
  © NVIDIA Corporation, CC-BY-4.0, https://huggingface.co/nvidia/parakeet-tdt-0.6b-v3; LiteRT
  conversion by Google, https://huggingface.co/litert-community/parakeet-tdt-0.6b-v3.
- **Removed with it:** the Whisper "terminal" bias (`WhisperDecoder.TERMINAL_VOCABULARY`,
  `prompt_mode: "terminal"`, `tai transcribe --terminal`). Voice input is plain dictation; an
  explicit OpenAI `prompt` (`tai transcribe --prompt "words"`) still biases Whisper.
