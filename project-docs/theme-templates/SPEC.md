...
| Template syntax | `{{ colors.<token>.<mode>.<format> }}` (spaces optional). Modes `default`, `dark`, `light` all resolve to the active palette. Formats: `hex` (`#rrggbb`), `hex_stripped`, `rgb` (`rgb(r, g, b)`), `rgba` (`rgba(r, g, b, 1.0)`), `red`, `green`, `blue` (decimal). `{{ mode }}` renders the mode. Unknown token, unknown format, or any `<*` block: the template is skipped and logged, nothing is written. Filters, loops, `image`, `closest_color`: out of scope. |
