...
|---|---|---|
| Kitty graphics animation and Unicode placeholders | M–L | The Tier 2 core — stored images (`a=t`), placements (`a=p`), crop, cell offsets, z-index, image numbers, and all delete forms — was delivered 2026-07-29 as slice 14 of `kitty-protocol-features.md`, verified end-to-end with yazi image previews. What remains of Tier 2/3: animation (`a=a/f/c`, needs a frame-driving render loop), Unicode placeholders (`U=1`, needs renderer placeholder decoding — the path to images inside tmux), and file/shm transmission (`t=f/t/s`, a security decision). Each answers a bounded `ENOSYS` today; wait for a packaged client that needs one. |
