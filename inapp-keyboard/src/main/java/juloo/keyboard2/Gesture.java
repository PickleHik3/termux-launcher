...
public final class Gesture
{
  ...
  State state;
  private final int circle_sensitivity;

  public Gesture(int starting_direction)
  {
    this(starting_direction, 2);
  }

  public Gesture(int starting_direction, int circle_sensitivity)
  {
    current_dir = starting_direction;
    state = State.Swiped;
    this.circle_sensitivity = Math.max(1, circle_sensitivity);
  }
  ...
  public boolean changed_direction(int direction)
  {
    ...
    {
      case Swiped:
        if (Math.abs(d) < circle_sensitivity)
          return false;
        // Start a rotation
        state = (clockwise) ?
