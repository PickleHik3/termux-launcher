...
public final class KeyboardData
{
  ...
  public static final int MAX_KEYS_PER_ROW = 32;
  public static final int MAX_KEYS_TOTAL = 512;
  ...
  public KeyboardData addExtraKeys(Iterator<Map.Entry<KeyValue, PreferredPos>> extra_keys)
  {
    ...
    /* Deviation from upstream: [add_key_to_pos] mutates the rows in place.
       KeyboardData instances are cached and shared in this project, so the
       working list is built from deep copies to never mutate shared rows. */
  ...
  {
    XmlResourceParser parser = res.getXml(R.xml.numpad);
    try
    {
      ...
      switch (parser.getName())
      {
        case "row":
          if (rows.size() >= MAX_ROWS)
            throw error(parser, "Layout exceeds the limit of " + MAX_ROWS + " rows");
          Row row = Row.parse(parser);
          totalKeys += row.keys.size();
          if (totalKeys > MAX_KEYS_TOTAL)
            throw error(parser, "Layout exceeds the limit of " + MAX_KEYS_TOTAL + " keys");
          rows.add(row);
          break;
        case "modmap":
          if (modmap != null)
  ...
  public static class Row
  {
    ...
    public static Row parse(XmlPullParser parser) throws Exception
    {
      ...
      {
        if (keys.size() >= MAX_KEYS_PER_ROW)
          throw error(parser, "Row exceeds the limit of "
              + MAX_KEYS_PER_ROW + " keys");
  ...
  private static float attribute_float(XmlPullParser parser, String attr,
      float default_val) throws ParseException
  {
    String val = parser.getAttributeValue(null, attr);
    if (val == null)
      return default_val;
    ...
    {
      throw error(parser, "Invalid float for '" + attr + "': " + val);
    }
  }

  /** Construct a parsing error. */
  private static ParseException error(XmlPullParser parser, String message)
  {
    return new ParseException(message, parser);
  }
  ...
  public static final class ParseException extends Exception
  {
    ...
    ParseException(String message, XmlPullParser parser)
    {
      super(message + " " + parser.getPositionDescription());
      lineNumber = parser.getLineNumber();
      columnNumber = parser.getColumnNumber();
      positionDescription = parser.getPositionDescription();
    }
  }
}
