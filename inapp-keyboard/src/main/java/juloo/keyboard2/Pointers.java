...
public final class Pointers implements Handler.Callback
{
  ...
  /** Set whether a key is latched or locked by adding a "fake" pointer, a
      ...
      This is used by the embedded view's programmatic state APIs.
  ...
  /** Commit every live pointer as if it was released instead of aborting.
      Embedded in an activity (not an IME window), the only source of
      ACTION_CANCEL is the system pilfering the pointer stream for a
      navigation gesture; aborting would silently drop the key the user was
      pressing or swiping. */
  public void onTouchCancelCommit()
  {
    // onTouchUp mutates _ptrs; collect real pointer ids first. Latched/fake
    // pointers (pointerId == -1) are state, not live touches — keep them.
    java.util.ArrayList<Integer> live = new java.util.ArrayList<Integer>();
    for (Pointer p : _ptrs)
      if (p.pointerId != -1)
        live.add(p.pointerId);
    for (int pointerId : live)
      onTouchUp(pointerId);
  }
  ...
  private void startLongPress(Pointer ptr)
  {
    ...
    _longpress_handler.sendEmptyMessageDelayed(what, _config.longPressTimeoutMs);
  }
  ...
  public final class Sliding
  {
    ...
    public void onTouchMove(Pointer ptr, float x, float y)
    {
      ...
      if (last_move_ms == -1)
      {
        if (travelled < (_config.swipeDistancePx + _config.sliderStepPx))
          return;
        last_move_ms = System.currentTimeMillis();
      }
      float current_speed = speed / _config.sliderStepPx;
