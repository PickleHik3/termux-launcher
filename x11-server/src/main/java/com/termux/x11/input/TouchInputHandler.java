...
public class TouchInputHandler {
    ...
    private final LorieHost mActivity;
    ...
    private TouchInputHandler(LorieHost activity, RenderData renderData,
...
//        mEdgeSlopInPx = ViewConfiguration.get(/*desktop*/ ctx).getScaledEdgeSlop();
    ...
    public TouchInputHandler(LorieHost activity, final InputEventSender injector) {
    ...
    public BiConsumer<Integer, Boolean> extractUserActionFromPreferences(Prefs p, String name) {
        ...
        switch(pref.asList().get()) {
            ...
            // "toggle additional key bar" and "open preferences" were upstream's own chrome: the
            // launcher has one keyboard and one settings screen, both reached its own way.
            case "toggle additional key bar":
            case "open preferences": return noAction;
            ...
            case "toggle fullscreen": return (key, down) -> {
                if (down) LorieHost.getPrefs().fullscreen.put(!LorieHost.getPrefs().fullscreen.get());
            };
    ...
    public PendingIntent extractIntentFromPreferences(Prefs p, String name, int requestCode) {
        ...
        switch(pref.asList().get()) {
            // Upstream's notification offers "open preferences" and "restart activity"; a home
            // screen has neither a preferences activity of its own to open nor a task to restart.
            case "exit":
            case "toggle soft keyboard":
            case "toggle additional key bar":
            case "release pointer and keyboard capture":
                return PendingIntent.getBroadcast(mActivity, requestCode, new Intent(LorieHost.ACTION_CUSTOM) {{
                    putExtra("what", name);
