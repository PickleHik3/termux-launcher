...
public class SamsungDexUtils {
    ...
    /**
     * Upstream passes its own Activity; the launcher's display is a page, not an activity, so the
     * component the meta-key capture is asked for is the launcher's own (see UPSTREAM.md).
     */
    static public void dexMetaKeyCapture(Context context, boolean enable) {
        if (!available())
            return;

        try {
            android.content.ComponentName component = context instanceof Activity
                ? ((Activity) context).getComponentName()
                : new android.content.ComponentName(context, context.getClass());
            requestMetaKeyEventMethod.invoke(manager, component, enable);
        } catch (Exception it) {
