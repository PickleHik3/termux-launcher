...
public class LorieView extends SurfaceView implements InputStub {
    ...
    private final InputConnection mConnection = new BaseInputConnection(this, false) {
        ...
        boolean replaceText(CharSequence newText, boolean reuse) {
            ...
            // Upstream unsets its own additional-keys bar's sticky modifiers here. The launcher
            // types into X through its own keyboard, so there is no second bar to unset.
            commitedText = true;
            return true;
        }
    ...
    void getDimensionsFromSettings(int width, int height) {
        Prefs prefs = LorieHost.getPrefs();
        int w = width;
        int h = height;
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        if (LorieHost.getPrefs().enforceCharBasedInput.get())
